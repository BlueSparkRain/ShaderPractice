Shader "Unlit/MyUnlitShader/SpecularMaskLightMode"
{
   Properties
    {
        ...
        _MaskTex("MaskTex",2D)=""{}
        _SpecularMaskScale("SpecularMaskScale",Range(0,10))=1
    }
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
               ...
               // float3 lambertColor=_LightColor0*albedo*max(0,dot(tangentNormal, normalize(i.lightDir)));
                ...
                float MaskNum=tex2D(_MaskTex,i.uv.xy).r*_SpecularMaskScale;
                float3 specularColor=_LightColor0*_SpecularColor*pow(max(0,dot(tangentNormal,halfDir)),_SpecularNum)*MaskNum;
